...
def round_cast(self, compiler, connection, **extra_context):
    # ROUND()'s value requires float arguments instead of decimal or integer,
    # but precision is left alone (as an integer).
    clone = self.copy()
    value, precision = self.get_source_expressions()
    value = (
        Cast(value, FloatField())
        if isinstance(value.output_field, (DecimalField, IntegerField))
        else value
    )
